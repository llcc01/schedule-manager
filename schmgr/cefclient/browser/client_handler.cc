...
bool ClientHandler::OnProcessMessageReceived(
  ...
  if (message_name == "schmgr_api") {
    core::back::OnScheduleManagerBackMessageReceived(browser, frame, message);
    return true;
  }

  if (message_name == "api_open_devtools")
  {
    auto args = message->GetArgumentList();
    if (args->GetString(0) != std::to_string(frame->GetIdentifier()))
    {
      return false;
    }
...
void ClientHandler::OnBeforeContextMenu(CefRefPtr<CefBrowser> browser,
  ...
  if ((params->GetTypeFlags() & (CM_TYPEFLAG_PAGE | CM_TYPEFLAG_FRAME)) != 0) {
    ...
    model->AddSeparator();
    model->AddItem(CLIENT_ID_SCHMGR, "Goto http://schmgr/");
    model->AddItem(CLIENT_ID_LOCALHOST, "Goto http://localhost:3000/");
    model->AddItem(CLIENT_ID_REMOTEHOST, "Goto http://schmgr.m3m0ry.space/");
  }
...
bool ClientHandler::OnContextMenuCommand(CefRefPtr<CefBrowser> browser,
  ...
  case CLIENT_ID_OFFLINE:
    ...
    return true;
  case CLIENT_ID_SCHMGR:
    browser->GetMainFrame()->LoadURL("http://schmgr/");
    return true;
  case CLIENT_ID_LOCALHOST:
    browser->GetMainFrame()->LoadURL("http://localhost:3000/");
    return true;
  case CLIENT_ID_REMOTEHOST:
    browser->GetMainFrame()->LoadURL("http://schmgr.m3m0ry.space/");
    return true;
